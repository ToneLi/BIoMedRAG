...
from .SplitInputsChunks import Get_Chunks_Inputs
from.ChuckWeights import  Chuck_weight
...
class Trainer:
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.label_smoother is not None and "labels" in inputs:
            ...
            labels = None
        # print("inputs",inputs)   #  {'input_ids   attention_mask  labels    tensor([[ 
        # outputs = model(**inputs)
       

        """---------1---redefine--chuck--loss--------"""
        # print("return_outputs",return_outputs) #False
        """
        medical llama13b: ▁Response: 13291
        llama2 7b: ▁Response: 11343
        llama2 13b:  ▁Response: 13291
        """
        Cmodel=Get_Chunks_Inputs()
        running_loss=0
        Inputs_chucks=Cmodel.redefine_more_ipnuts(ids=inputs["input_ids"][0])

        Input_relation_SIms=[]
        for I_chuck in Inputs_chucks:
            # print("I_chuck",I_chuck)

            reponse_index=((I_chuck["input_ids"][0] == 13291).nonzero(as_tuple=True)[0][-1]) # 11343
            outputs = model(**I_chuck)

            matri=outputs["logits"]
            input_vector=torch.mean(matri[:,:reponse_index+1,:],dim=1)
            relation_vector=torch.mean(matri[:,reponse_index+1:,:],dim=1)
            Cos_=cos(input_vector,relation_vector)
            Input_relation_SIms.append(Cos_)
        ...
        # print("Input_relation_SIms",torch.max(Input_relation_SIms))
        max_cos=Input_relation_SIms[0]
        # print("Input_relation_SIms",Input_relation_SIms)
        for co in Input_relation_SIms:
            if co>max_cos:
                max_cos=co
        # print(torch.topk(torch.tensor(Input_relation_SIms),2))
        # indices_=torch.topk(torch.tensor(Input_relation_SIms),2)[1]
        # max_cos=(Input_relation_SIms[indices_[0]]+Input_relation_SIms[indices_[1]])/2

        # print("max_cos",max_cos)
        """---------2 chuck weight-----"""
        # print("max_cos",max_cos)
        Cmodel=Chuck_weight()
        weight_model, weight=Cmodel.get_weight_value(ids=inputs["input_ids"][0])
        sime_list=weight.squeeze(0)[0,1:]

        # print("sime_list------",sime_list)
        chuck_max_sim=torch.max(sime_list)

        """----3 model save----------"""
                
      
        no_update = 0
        best_model = weight_model.state_dict()
        best_score = -float("inf")
        # print("max_cos",max_cos)
        # print("chuck_max_sim[0]",chuck_max_sim)
        global_results=-(max_cos[0]-chuck_max_sim)
        eps = 0.0001
        if global_results > best_score + eps:
            ...
            checkpoint_file_name = checkpoint_path + suffix + "llama2_13b_n_5_top1_1000.pt"
        #   logging.info('Saving checkpoint to %s' % checkpoint_file_name)
            torch.save(weight_model.state_dict(), checkpoint_file_name)

      

        running_loss=running_loss+(max_cos[0]-chuck_max_sim)

        return (running_loss, outputs) if return_outputs else running_loss
